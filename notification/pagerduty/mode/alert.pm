...
package notification::pagerduty::mode::alert;
...
sub new {
    ...
    $options{options}->add_options(arguments => {
        "hostname:s"            => { name => 'hostname', default => 'api.pagerduty.com' },
        "port:s"                => { name => 'port', default => 443 },
        "proto:s"               => { name => 'proto', default => 'https' },
        "urlpath:s"             => { name => 'url_path', default => "/incidents" },
        "account-pagerduty:s"   => { name => 'account'}
        "service-token:s"       => { name => 'service_token' },
...
sub check_options {
    ...
    if (!defined($self->{option_results}->{service_token})) {
        $self->{output}->add_option_msg(short_msg => "You need to set --service-token option");
    ...
    if (!defined($self->{option_results}->{account}) || $self->{option_results}->{account} eq '') {
        $self->{output}->add_option_msg(short_msg => "You need to specify --account-pagerduty option.");
        $self->{output}->option_exit();
    }
...
sub format_payload {
    ...
    my $payload = { parse_mode => 'HTML',
                    type => 'Incident',
                    title => $self->{option_results}->{host_name}
                    service =>{
...
sub run {
    ...
    $self->{http}->add_header(key => 'Authorization', value => 'Token token='. $self->{option_results}->{service_token});
